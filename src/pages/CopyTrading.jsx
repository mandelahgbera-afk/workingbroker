...
export default function CopyTrading() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [tradersRes, copiesRes] = await Promise.all([
          supabase.from('copy_traders').select('*').eq('is_approved', true),
          supabase.from('copy_trade').select('*').eq('user_email', user.email).eq('is_active', true),
        ]);
        setTraders(tradersRes.data || []);
        setMyCopies(copiesRes.data || []);
      } catch (error) {
        console.error('[v0] Error fetching copy trading data:', error);
  ...
  const handleCopy = async () => {
    ...
    try {
      const { error } = await supabase.from('copy_trade').insert({
      ...
      const { data: updated } = await supabase.from('copy_trade').select('*').eq('user_email', user.email).eq('is_active', true);
      setMyCopies(updated || []);
      toast.success(`Now copying ${copyModal.trader_name}!`);
      setCopyModal(null);
      setAlloc("");
    } catch (error) {
      console.error('[v0] Error copying trader:', error);
      toast.error('Failed to start copying trader');
    }
  };

  const handleStop = async (copy) => {
    try {
      const { error } = await supabase.from('copy_trade').update({ is_active: false }).eq('id', copy.id);
      if (error) throw error;
      setMyCopies(prev => prev.filter(c => c.id !== copy.id));
      toast.success("Stopped copying trader");
    } catch (error) {
      console.error('[v0] Error stopping copy trader:', error);
