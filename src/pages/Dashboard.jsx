...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [balRes, portRes, cryptoRes, txRes] = await Promise.all([
          ...
          supabase.from('transaction').select('*').eq('user_email', user.email).order('created_date', { ascending: false }).limit(5),
        ]);
        
        setBalance(balRes.data?.[0] || { balance_usd: 0, total_invested: 0, total_profit_loss: 0 });
