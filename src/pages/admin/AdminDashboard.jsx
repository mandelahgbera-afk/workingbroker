...
export default function AdminDashboard() {
  ...
  const load = async () => {
    try {
      const [usersRes, txnsRes] = await Promise.all([
        ...
        supabase.from('transaction').select('*').order('created_date', { ascending: false }).limit(200),
      ]);
    ...
    txns.forEach(t => {
      const m = months[new Date(t.created_date).getMonth()];
  ...
  const handleApprove = async (tx) => {
    try {
      await supabase.from('transaction').update({ status: "approved" }).eq('id', tx.id);
  ...
  const handleReject = async (tx) => {
    try {
      await supabase.from('transaction').update({ status: "rejected" }).eq('id', tx.id);
            ...
            {pending.map(tx => (
              <div key={tx.id} className="flex items-center gap-4 px-5 py-4 hover:bg-secondary/30 transition-colors">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground pl-3.5">{tx.user_email} • {new Date(tx.created_date).toLocaleDateString()}</p>
                </div>
