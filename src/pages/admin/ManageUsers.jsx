...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [balanceRes, portfolioRes, cryptosRes, txnsRes] = await Promise.all([
          supabase.from('user_balance').select('*').eq('user_email', user.email),
          supabase.from('portfolio').select('*').eq('user_email', user.email),
          supabase.from('cryptocurrencies').select('*').eq('is_active', true),
          supabase.from('transactions').select('*').eq('user_email', user.email).order('created_date', { ascending: false }).limit(5),
        ]);
        setBalance(balanceRes.data?.[0] || { balance_usd: 0, total_invested: 0, total_profit_loss: 0 });
